import React, { useState, useEffect } from 'react';
import {
  Leaf,
  TrendingDown,
  Truck,
  Activity,
  RefreshCw,
  Award,
  FileSpreadsheet,
  Search,
  CheckCircle2,
  TreeDeciduous,
  ShieldCheck,
  Zap,
  Filter
} from 'lucide-react';
import { carbonApi } from '../api';

export default function CarbonPage() {
  const [data, setData] = useState(null);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [ratingFilter, setRatingFilter] = useState('ALL');

  async function loadData() {
    setLoading(true);
    try {
      const res = await carbonApi.getMetrics();
      setData(res);
    } catch (err) {
      console.error('Error fetching carbon metrics', err);
    } finally {
      setLoading(false);
    }
  }

  useEffect(() => {
    loadData();
  }, []);

  const exportCSV = () => {
    if (!data || !data.vehicleBreakdown) return;

    let csv = 'Registration,Model,Driver,FuelType,EstCO2Tonnes,Rating\n';
    data.vehicleBreakdown.forEach((v) => {
      csv += `"${v.registration}","${v.makeModel}","${v.driver}","${v.fuelType || 'Diesel (BS-VI)'}",${v.co2Tonnes},"${v.rating}"\n`;
    });

    const blob = new Blob([csv], { type: 'text/csv' });
    const url = window.URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `smartfleet_carbon_esg_report_${Date.now()}.csv`;
    a.click();
  };

  if (loading || !data) {
    return (
      <div className="p-12 text-center text-slate-400 text-xs flex flex-col items-center justify-center gap-3">
        <Activity className="w-8 h-8 text-emerald-400 animate-spin" />
        <span className="font-semibold text-slate-300">
          Calculating dynamic fleet ESG carbon intensity & fuel emissions...
        </span>
      </div>
    );
  }

  const vehicles = data.vehicleBreakdown || [];
  const filteredVehicles = vehicles.filter((v) => {
    const q = search.toLowerCase();
    const matchesSearch =
      (v.registration || '').toLowerCase().includes(q) ||
      (v.makeModel || '').toLowerCase().includes(q) ||
      (v.driver || '').toLowerCase().includes(q);
    const matchesRating = ratingFilter === 'ALL' || v.rating === ratingFilter;
    return matchesSearch && matchesRating;
  });

  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="bg-slate-900/90 border border-slate-800 p-6 rounded-2xl shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-black text-slate-100 flex items-center gap-2">
            <Leaf className="w-6 h-6 text-emerald-400" /> Carbon Footprint & ESG Analytics
          </h1>
          <p className="text-xs text-slate-400 mt-1">
            Real-time ESG sustainability tracking, dynamic fleet CO₂ emissions, and vehicle-level carbon intensity
          </p>
        </div>

        <div className="flex items-center gap-2">
          <button
            onClick={loadData}
            className="p-2.5 bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-700 font-bold rounded-xl text-xs flex items-center gap-1.5 transition-all"
            title="Refresh Carbon Telemetry"
          >
            <RefreshCw className="w-4 h-4 text-emerald-400" />
          </button>
          <button
            onClick={exportCSV}
            className="px-4 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-xl text-xs flex items-center gap-2 shadow-lg shadow-emerald-600/20 transition-all"
          >
            <FileSpreadsheet className="w-4 h-4" /> Export ESG Report
          </button>
        </div>
      </div>

      {/* KPI Cards */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-slate-900/80 border border-slate-800 p-5 rounded-2xl shadow-lg">
          <span className="text-[11px] text-slate-400 block font-semibold">Total Estimated CO₂ (This Month)</span>
          <span className="text-2xl font-black text-emerald-400">{data.totalCO2Tonnes} Tonnes</span>
          <p className="text-[10px] text-slate-500 mt-1">
            DEF/Diesel standard: {data.emissionFactorKgPerLitre || 2.68} kg CO₂/L
          </p>
        </div>

        <div className="bg-slate-900/80 border border-slate-800 p-5 rounded-2xl shadow-lg">
          <span className="text-[11px] text-slate-400 block font-semibold">Average CO₂ / Vehicle</span>
          <span className="text-2xl font-black text-slate-100">{data.avgCO2PerVehicle} Tonnes</span>
          <p className="text-[10px] text-slate-500 mt-1">
            Across {vehicles.length} evaluated operational assets
          </p>
        </div>

        <div className="bg-slate-900/80 border border-slate-800 p-5 rounded-2xl shadow-lg">
          <span className="text-[11px] text-slate-400 block font-semibold">CO₂ Reduction vs Previous Month</span>
          <span className="text-2xl font-black text-amber-400 flex items-center gap-1">
            <TrendingDown className="w-5 h-5 text-emerald-400" /> -{data.reductionVsLastMonth}%
          </span>
          <p className="text-[10px] text-emerald-400 mt-1">AI route optimization efficiency gain</p>
        </div>

        <div className="bg-slate-900/80 border border-slate-800 p-5 rounded-2xl shadow-lg">
          <span className="text-[11px] text-slate-400 block font-semibold">Tree Carbon Offset Equivalent</span>
          <span className="text-2xl font-black text-cyan-400 flex items-center gap-1.5">
            <TreeDeciduous className="w-5 h-5 text-cyan-400" />
            {data.treeEquivalents} Trees
          </span>
          <p className="text-[10px] text-slate-500 mt-1">22 kg CO₂ annual absorption capacity</p>
        </div>
      </div>

      {/* Green Fleet Sustainability Progress Banner */}
      <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-3">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            <ShieldCheck className="w-5 h-5 text-emerald-400" />
            <div>
              <h3 className="text-sm font-bold text-slate-100">Green Fleet ESG Index Score</h3>
              <p className="text-[10px] text-slate-400">Targeting zero-emission corridor certification</p>
            </div>
          </div>
          <span className="text-lg font-black text-emerald-400 font-mono">
            {data.greenFleetScore || 86} / 100 ESG Score
          </span>
        </div>

        <div className="w-full bg-slate-950 rounded-full h-2.5 overflow-hidden border border-slate-800">
          <div
            className="bg-gradient-to-r from-emerald-500 to-cyan-400 h-full rounded-full transition-all duration-500"
            style={{ width: `${data.greenFleetScore || 86}%` }}
          />
        </div>
      </div>

      {/* Filter and Search Bar */}
      <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-4 shadow-lg grid grid-cols-1 md:grid-cols-3 gap-3">
        <div className="md:col-span-2 relative">
          <Search className="w-4 h-4 text-slate-400 absolute left-3.5 top-3" />
          <input
            type="text"
            placeholder="Search by vehicle registration, model, or driver..."
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            className="w-full pl-10 pr-4 py-2 bg-slate-950/80 border border-slate-700/80 rounded-xl text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:border-amber-500"
          />
        </div>

        <div>
          <select
            value={ratingFilter}
            onChange={(e) => setRatingFilter(e.target.value)}
            className="w-full py-2 px-3 bg-slate-950/80 border border-slate-700/80 rounded-xl text-xs text-slate-200 focus:outline-none focus:border-amber-500"
          >
            <option value="ALL">All ESG Ratings</option>
            <option value="Green A+">Green A+ (Eco-Friendly)</option>
            <option value="Efficient A">Efficient A (Optimized)</option>
            <option value="Standard B">Standard B (Normal Fleet)</option>
            <option value="High Emission C">High Emission C (Needs Service)</option>
          </select>
        </div>
      </div>

      {/* Vehicle Carbon Intensity Breakdown */}
      <div className="bg-slate-900/90 border border-slate-800 rounded-2xl overflow-hidden shadow-xl">
        <div className="p-4 border-b border-slate-800 flex items-center justify-between">
          <h3 className="text-sm font-bold text-slate-100 flex items-center gap-2">
            <Truck className="w-4 h-4 text-emerald-400" /> Vehicle-by-Vehicle Carbon Intensity
          </h3>
          <span className="text-[10px] font-mono text-slate-400">
            {filteredVehicles.length} of {vehicles.length} Assets Shown
          </span>
        </div>

        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs text-slate-300">
            <thead className="bg-slate-950/80 text-slate-400 uppercase text-[10px] font-bold tracking-wider border-b border-slate-800">
              <tr>
                <th className="p-4">Vehicle Reg</th>
                <th className="p-4">Model & Fuel</th>
                <th className="p-4">Assigned Driver</th>
                <th className="p-4">Odometer Run</th>
                <th className="p-4">Estimated CO₂</th>
                <th className="p-4">ESG Rating</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-800/80">
              {filteredVehicles.length === 0 ? (
                <tr>
                  <td colSpan={6} className="p-8 text-center text-slate-400">
                    No vehicles found matching the filter criteria.
                  </td>
                </tr>
              ) : (
                filteredVehicles.map((v) => (
                  <tr key={v.id} className="hover:bg-slate-800/40 transition-colors">
                    <td className="p-4 font-bold text-slate-100 font-mono">{v.registration}</td>
                    <td className="p-4">
                      <div className="text-slate-200 font-medium">{v.makeModel}</div>
                      <div className="text-[10px] text-slate-500">{v.fuelType || 'Diesel (BS-VI)'}</div>
                    </td>
                    <td className="p-4 text-slate-400">{v.driver}</td>
                    <td className="p-4 text-slate-300 font-mono">
                      {(v.odometer || 0).toLocaleString()} km
                    </td>
                    <td className="p-4 font-mono font-bold text-amber-400">{v.co2Tonnes} Tonnes</td>
                    <td className="p-4">
                      <span
                        className={`px-2.5 py-1 rounded-lg text-[10px] font-black uppercase ${
                          v.rating.includes('A+')
                            ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30'
                            : v.rating.includes('A')
                            ? 'bg-cyan-500/20 text-cyan-400 border border-cyan-500/30'
                            : v.rating.includes('B')
                            ? 'bg-amber-500/20 text-amber-400 border border-amber-500/30'
                            : 'bg-rose-500/20 text-rose-400 border border-rose-500/30'
                        }`}
                      >
                        {v.rating}
                      </span>
                    </td>
                  </tr>
                ))
              )}
            </tbody>
          </table>
        </div>
      </div>
    </div>
  );
}
